...
struct polygon
{
    ...
    point p[10];
    line l[maxp];
    polygon(int _n,point a,point b,point c,point d)
    {
        n=_n;p[0]=a,p[1]=b,p[2]=c,p[3]=d;
    }
    void input(int _n,int arx[],int ary[]){
        n=_n;
        for (int i=0;i<n;i++)   p[i].x=arx[i],p[i].y=ary[i];
    }
