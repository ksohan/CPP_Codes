...
// inline lll HASH(lll oldhash,lll base,int szz,int bad,int jog){lll newhash=base*(oldhash-(bad*bpow(base,(lll)szz-1)))+jog; return newhash;}
...
string manachar(string s){
	...
	string pal="";
	for(int i=center-mxlen+1;i<=center+mxlen-1;i++){
		if(s[i]!='#') pal+=s[i];
	}
	return pal;
}
